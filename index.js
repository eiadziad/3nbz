...
/* if (firstWord == '❌') {
    ...
    if(firstWord == 'لغز؟') {
        ...
        if (message.includes('@3NB77'!== 'nightbot')) {
         ...
         if(firstWord == 'عنب' && user.username.toLowerCase() !== 'nightbot') {
          client.say(channel, `@${user.username} ${list3[Math.floor(Math.random()*list3.length)]}`);
      }
      if(firstWord == '@3nb77' && user.username.toLowerCase() !== 'nightbot') {
          client.say(channel, `@${user.username} ${list3[Math.floor(Math.random()*list3.length)]}`);
      }
      if (firstWord === '@3NB77' && user.username.toLowerCase() !== 'nightbot') {
        client.say(channel, `@${user.username} ${list3[Math.floor(Math.random() * list3.length)]}`);
...
const wordsList2 = [
  ...
  'NEON GENESIS EVANGELION', 'FAIRYTAIL', 'POKEMON', 'ONEPUNCHMAN', 'HUNTERXHUNTER',
  'COWBOYBEBOP', 'BLACKCLOVER', 'DEMONSLAYER', 'BLENDERS', 'JOJOS BIZARRE ADVENTURE',
  'REZERO', 'GURRENLAGANN', 'STEINSGATE', 'ATTACKONTITAN', 'YOUR NAME',
  ...
  'FIRE FORCE', 'MOB PSYCHO 100', 'HIGH SCORE GIRL', 'ERASED', 'DOROHEDORO',
